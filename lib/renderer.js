...
module.exports = function (context) {
	...
	vueContext.provision = true;
	const entry = eval(vueContext.bundle.files[vueContext.bundle.entry]).default(vueContext);
	delete vueContext.provision;
	if (entry.generate) entry.generate(context)

	const data = typeof entry.data === 'function'
		? entry.data.call(context)
		: entry.data || {};
	...
	return new Promise((resolve, reject) => {
		...
		bundleRenderer.renderToString(vueContext, (err, html) => {
			if (err) {
				return reject(new Error(yaml.dump(err)));
			}
			if (vueContext.state) {
